...
async function handleProxy(request) {
  ...
  try {
    ...
    const isM3U8 = contentType.toLowerCase().includes('mpegurl') || targetUrl.pathname.toLowerCase().includes('.m3u8');
...
function rewriteM3U8(text, sourceUrl, origin) {
  ...
  return text.split('\n').map((line) => {
    ...
    if (trimmed.startsWith('#')) return rewriteUriAttributes(line, baseUrl, proxyBase);
